...
class RCAN:
    ...
    def srcnn(self, x, f, kernel_size, reduction, use_bn, scale):
        with tf.variable_scope("Srcnn_Network"):
            ...
            body += head  # tf.math.add(body, head)
            # 3. tail
            x = tfu.conv2d(x, f=self.n_channel, k=kernel_size, name="conv2d-forth")  # (-1, 384, 384, 3)
            # x = self.image_processing(tail, sign=1, name='post-processing')
            return x
    ...
    def build_model(self):
        ...
        # metrics
        self.psnr = tf.reduce_mean(metric.psnr(self.x_hr, self.output, m_val=255))
        self.ssim = tf.reduce_mean(metric.ssim(self.x_hr, self.output, m_val=255))
